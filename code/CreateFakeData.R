CreateFakeDataForOneMunicipality <- function(seed=4) {
  skeleton <- data.table(date = seq.Date(as.Date("2000-01-01"), as.Date("2010-12-31"), by = 1))
  ...
  skeleton[month==4, monthEffect:= 0.4]
  skeleton[month==5, monthEffect:= 0.8]
  skeleton[month==6, monthEffect:= 1.0]
  skeleton[month==7, monthEffect:= 1.2]
  skeleton[month==8, monthEffect:= 1.0]
  skeleton[month==9, monthEffect:= 0.6]
  skeleton[month==10, monthEffect:= 0.4]
  ...
  skeleton[, mu := exp(1.2 + 0.01*(year-2000)) + monthEffect + 2*outbreak]
...
CreateFakeData <- function(){
  norwayLocations <- readRDS("data_raw/norwayLocations.RDS")
  municipalities <- norwayLocations$municip
  ...
  for(i in seq_along(municipalities)){
    ...
    data[[i]][,location:=municipalities[i]]
  }
  data <- rbindlist(data)
